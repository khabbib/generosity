...
app.post('/paymentrequests', (req, res) => {
  ...
  const options = requestOptions(
    'POST',
    `${agent.host}/api/v1/paymentrequests`,
    data
  );
...
app.get('/paymentrequests/:requestId', (req, res) => {
  const options = requestOptions(
    'GET',
    `${agent.host}/api/v1/paymentrequests/${req.params.requestId}`
  );
...
app.get('/refunds/:refundId', (req, res) => {
  const options = requestOptions(
    'GET',
    `${agent.host}/api/v1/refunds/${req.params.refundId}`
  );
    ...
    if (response.statusCode == 200) {
      res.json({
        ...
        originalPaymentReference:
          response.body['originalPaymentReference'] || '',
        status: response.body['status'],
      });
    } else {
      res.send(body);
